#################230115 repeatation for pt11_pt14 qpcr results
library(readxl)
...
P_left1 <- ggplot(tdtomato_ct_left, aes(x=group, y=values,fill=tissue)) +
  geom_col(alpha=.3)+
  theme_classic()+
  scale_y_break(c(1e+05,1e+7),scales = 2,ticklabels = c(1e+07,7e+09))+
...
P_right1 <- ggplot(tdtomato_ct_right, aes(x=group, y=values,fill=tissue)) +
  geom_col(alpha=.3)+
  theme_classic()+
  scale_y_break(c(1e+04,1e+5),scales = 1.5,ticklabels = c(1e+04,1e+05))+
  theme(legend.position = 'top',
        axis.text.x = element_text(face = 'bold',size = 12),
        axis.text.y = element_text(face = 'bold',size = 12),
        plot.title = element_text(face = 'bold',size = 20),
...
ggsave('230115_p1a_rl.png',
       units = 'cm',
       dpi = 600,width = 38,height = 26)
######################################
ggplot(tdtomato_ct_right, aes(x=group, y=values,fill=tissue)) +
  geom_col()+
  scale_y_log10()

p1a_left <- ggplot(tdtomato_ct_left, aes(x=group, y=values,fill=tissue)) + 
  geom_bar(stat="summary",  color="grey", binwidth=2,#stat="identity", 
...
p1a_Right <- ggplot(tdtomato_ct_right, aes(x=group, y=values,fill=tissue)) + 
...
p1a_Right
library(ggpubr)
ggarrange(p1a_left,p1a_Right,ncol = 1,
          labels = c('A','B'),heights = c(1,.6),
          font.label = list(size=22,color='red',face='bold'))
ggsave('230115_p1a_rl.png',
       units = 'cm',
...
##############################
options(scipen = 1)
format(tdtomato_ct_right$values)
format(tdtomato_ct_left$values)
...
ggarrange(p1_Left_form,p1_Right_form,ncol = 1,
          common.legend = T,#legend.position='bottom',
          labels = c('A','B'),heights = c(1,.6),
          font.label = list(size=22,color='red',face='bold'))
ggsave('230115_p1rl_form.png',
       units = 'cm',
       dpi = 600,width = 38,height = 26)
#####################################
...
p1d <- ggplot(tdtomato_ct, aes(x=Incubation, y=values,fill=Formulation,color=tissue))+ 
  ...
  geom_point(aes(color=tissue),#palette=c('orange','blue'),
             ...
             size=3)+
  scale_color_manual(values = c('Muscle_L'='orange','Muscle_R'='blue'))
p1d####make no sense for that plot


library(ggpubr)
ggarrange(p1b,p1c,ncol = 1,
          #common.legend = T,
          labels = c('C','D'),
          font.label = list(size=22,color='red',face='bold'))
